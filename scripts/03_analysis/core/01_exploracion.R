...
# English display names for country codes (figures / facets)
...
paises <- unique(familias$pais_persona)
...
familias_conexiones <- bind_rows(lapply(resultados_pais, function(r) r$nodos)) %>%
  ...
  ungroup() %>%
  mutate(
    pais_label = country_name_en(pais),
...
if (nrow(top_conectores_df) > 0) {
  ...
  p_barras <- ggplot(top_conectores_df, aes(x = fct_reorder(nombre_corto, grado), y = grado, fill = pais_label)) +
    ...
    labs(
      x = NULL,
      y = "Number of connections (related persons)",
      title = "Highest within-country connectors by country",
      subtitle = "Related-profile network (parents, spouse, children, siblings, family); within-country ties only."
    ) +
  ...
  if (nrow(familias_conexiones) > 0) {
    p_familias <- ggplot(familias_conexiones, aes(x = fct_reorder(familia_corto, total_conexiones), y = total_conexiones, fill = pais_label)) +
      ...
      labs(
        x = NULL,
        y = "Total connections (sum of member degrees)",
        title = "Families with most within-country connections",
        subtitle = "Per family: sum of ties members contribute to the related-profile network."
      ) +
      theme_minimal(base_size = 10) +
      theme(strip.text = element_text(face = "bold"), plot.title = element_text(face = "bold"), panel.grid.major.y = element_blank())
...
for (p in paises) {
  ...
  ed <- ed %>%
    ...
    mutate(pais = p, pais_label = country_name_en(p))
...
if (nrow(all_edges_plot) > 0) {
  ...
  p_facet <- p_facet +
    ...
    labs(
      title = "Family kinship networks by country",
      subtitle = "Exploratory layout; node size = degree within each subgraph."
    ) +
  ...
  for (p in paises) {
    ...
    p_pais <- ggraph(g_tbl, layout = "manual", x = x, y = y) +
      geom_edge_link(alpha = 0.4, color = "gray50", linewidth = 0.35, lineend = "round") +
      ...
      scale_color_manual(values = familia_colors, name = "Family", na.value = "gray50") +
      scale_size_continuous(range = c(1, 6), name = "Degree") +
      labs(title = paste0("Family network - ", pais_lab), subtitle = paste0(nrow(nodos_p), " nodes, ", nrow(edges_p), " edges. Color = family.")) +
...
# Selección manual para ver en el Viewer
paises_ver <- c("chile", "argentina")  # ← cambia aquí los que quieras

library(patchwork)
library(ggrepel)

plots_ver <- map(paises_ver, function(p) {
  r <- resultados_pais[[p]]
  if (is.null(r$edges) || nrow(r$edges) == 0) return(NULL)
  
  # Top nodos por grado (etiquetas ggrepel)
  top15 <- r$nodos %>%
    arrange(desc(grado)) %>%
    slice_head(n = 8) %>%
    mutate(nombre_corto = if_else(nchar(nombre) > 25, paste0(str_sub(nombre, 1, 22), "..."), nombre))
  
  g_tbl <- as_tbl_graph(r$grafo) %>%
    activate(nodes) %>%
    left_join(r$nodos %>% select(url, x, y, nombre, grado, familia),
              by = c("name" = "url"))
  
  ggraph(g_tbl, layout = "manual", x = x, y = y) +
    # Aristas visibles en Viewer (antes ~invisibles con alpha 0.15)
    geom_edge_link(
      alpha = 0.42,
      color = "gray45",
      linewidth = 0.38,
    ...
    geom_node_point(aes(size = grado + 0.5, color = familia), alpha = 0.7) +
    # Etiquetas solo del top (mismo subset que top15 / slice_head arriba)
    geom_text_repel(
      data = top15,
    ...
    scale_color_manual(values = familia_colors, na.value = "gray50") +
    scale_size_continuous(range = c(0.8, 5)) +
    labs(title = country_name_en(p)) +
...
wrap_plots(plots_ver, ncol = length(plots_ver))
...
for (i in seq_len(nrow(familias))) {
  pais_persona <- trimws(tolower(as.character(familias$pais_persona[i])))
...
if (nrow(edges_paises) >= 1L) {
  ...
  v_df <- tibble(
    ...
    mutate(pais_label = country_name_en(pais))
  ...
  p_cross <- ggplot() +
    ...
    scale_linewidth_continuous(range = c(0.3, 2.5), name = "Ties (kin. + dipl.)") +
    labs(
      title = "Cross-national ties: family and diplomatic/professional connections",
      subtitle = "Edges = persons with family in another country or biographical link (death, residence, work). Width = count."
    ) +
    theme_void(base_size = 11) +
    theme(
...
# ============================================================
# FAMILIAS CON MÁS PODER DE ENLACE INTERNACIONAL
# ============================================================

# 1) Unir las aristas transnacionales con la familia del nodo origen
poder_internacional <- cross_familia %>%
  left_join(
    familias %>% distinct(url, familia, nombre),
    by = c("from" = "url")
  ) %>%
  filter(!is.na(familia), familia != "Sin familia", familia != "") %>%
  group_by(familia) %>%
  summarise(
    n_vinculos       = n(),                                          # total aristas cross-country
    n_paises_destino = n_distinct(pais_destino),                    # cuántos países alcanza
    n_personas       = n_distinct(from),                            # cuántos miembros tienen vínculos externos
    paises_destino   = paste(sort(unique(pais_destino)), collapse = ", "),
    .groups = "drop"
  ) %>%
  arrange(desc(n_vinculos))

# Ver top 20
print(poder_internacional %>% slice_head(n = 20))

# 2) Gráfico: top 20 familias por vínculos internacionales
top_fam_int <- poder_internacional %>% slice_head(n = 20)

ggplot(top_fam_int, aes(
  x = fct_reorder(familia, n_vinculos),
  y = n_vinculos,
  fill = n_paises_destino
)) +
  geom_col() +
  geom_text(
    aes(label = paste0(n_paises_destino, " countries")),
    hjust = -0.1, size = 3, color = "gray30"
  ) +
  coord_flip() +
  scale_fill_gradient(low = "#c6dbef", high = "#084594", name = "Countries\nreached") +
  scale_y_continuous(expand = expansion(mult = c(0, 0.2))) +
  labs(
    title = "Families with greatest international bridging capacity",
    subtitle = "By number of kinship ties crossing national borders",
    x = NULL,
    y = "Number of transnational ties"
  ) +
